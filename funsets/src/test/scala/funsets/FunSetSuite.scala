...
class FunSetSuite extends FunSuite {
  ...
  test("union contains all elements of each set") {
    new TestSets {
      ...
      val ns = union(s, s1)
      assert(contains(ns, 1), "Union 1")
      assert(contains(ns, 2), "Union 2")
    }
  }
    ...
    test("intersection"){
      new TestSets {
        ...
        assert(!contains(intersect(s, s1), 2), "Intersection 2")
      }
    }
    ...
    test("filter") {
      new TestSets {
        val s = union(s1, s2)
        val p = filter(s, x => x == 1)
        assert(contains(p, 1), "filter 1")
        assert(!contains(p, 2), "filter 2")
        assert(!contains(p, 100), "filter 100")
      }
    }
    ...
    test("map") {
      new TestSets {
         ...
         assert(contains(s, 4), "contains 4")
